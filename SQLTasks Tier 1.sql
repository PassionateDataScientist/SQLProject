...
the member name. */
Please check this one. Can I use || instead of concat?
...
SELECT m.firstname || ' ' || m.surname AS Member_name, f.name AS Facility_name,
CASE
WHEN m.memid =0
...
INNER JOIN Bookings AS b ON m.memid = b.memid
INNER JOIN Facilities AS f ON b.facid = f.facid
WHERE (b.starttime > '2012-09-13' AND b.starttime < '2012-09-15')
...
SELECT Member_name, Facility_name, cost FROM(
    ...
    INNER JOIN Bookings AS b ON m.memid = b.memid
    INNER JOIN Facilities AS f ON b.facid = f.facid
    WHERE b.starttime > '2012-09-13' AND b.starttime < '2012-09-15') AS Booking
WHERE cost >30
ORDER BY cost DESC;
